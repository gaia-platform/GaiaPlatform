...
```bash
export GAIA_REPO=<path_to_your_repors>/GaiaPlatform
```
...
Start with the `[apt]` section in `$GAIA_REPO/production/gdev.cfg`. Install all the packages with `apt install`:
