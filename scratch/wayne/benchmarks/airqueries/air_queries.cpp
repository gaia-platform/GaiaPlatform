...
// Query #2:  Describe the 1 hop routes between [SEA] and [OTP].
int32_t query_SEA_to_OTP_1_hop(vector<string>& req)
{
    uint32_t i = 0;
...
int main ()
{
    ...
    for (;;) {
        printf("1. Find the 1 stop connection airports between [SEA] and [OTP].\n");
        printf("2. Describe the 1 hop routes between [SEA] and [OTP].\n");
        ...
        printf("5. Describe the 1 hop routes between SEA and OTP that avoid Tarom to OTP.\n");
        printf("q. Quit\n");
        printf("> ");
        ...
        switch (req[0][0]) {
            ...
            case '2':
            PerfTimer(ns, [&]() {
                rows = query_SEA_to_OTP_1_hop(req);
            });
            printf("query_SEA_to_OTP_1_hop: produced %u rows in %.2f msec\n", rows, PerfTimer::ns_ms(ns));
            total_ns += ns;
            break;
