...
public:
  ...
  {
    s_data = (data*)mmap(
      nullptr, sizeof(data), PROT_READ | PROT_WRITE, MAP_SHARED, s_fd_data, 0);

    if (MAP_FAILED == s_data) {
      throw_runtime_error("mmap failed");
    }

    s_log = (log*)mmap(nullptr,
                       sizeof(log),
    ...
    s_offsets = (offsets*)mmap(nullptr,
                               sizeof(offsets),
                               PROT_READ | PROT_WRITE,
...
protected:
  ...
  static void allocate_object(int64_t row_id, size_t size)
  {
    ...
    (*s_offsets)[row_id] =
      1 + __sync_fetch_and_add(&s_data->objects[0],
...
public:
                                       ...
                                       bool log_updates = true)
  {
    gaia_ptr<gaia_se_node> node(
      id, payload_size + sizeof(gaia_se_node), false, log_updates);
...
struct gaia_se_edge
{
                                       ...
                                       bool log_updates = true)
  {
    ...
    gaia_ptr<gaia_se_edge> edge(
      id, payload_size + sizeof(gaia_se_edge), true, log_updates);
...
template<>
inline void
gaia_ptr<gaia_se_node>::remove(gaia_ptr<gaia_se_node>& node)
{
  if (!node) {
    return;
  }
...
template<>
inline void
gaia_ptr<gaia_se_edge>::remove(gaia_ptr<gaia_se_edge>& edge)
{
  if (!edge) {
    return;
  }
