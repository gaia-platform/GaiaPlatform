...
public:
  ...
  static void tx_begin() {
    ...
    s_offsets =
        (offsets *)mmap(nullptr, sizeof(offsets), PROT_READ | PROT_WRITE,
  ...
  template <typename T> friend class gaia_ptr;
  template <typename T> friend class gaia_iterator;
...
protected:
  ...
  static const char *make_shm_name(std::string &s, const char *prefix,
                                   const char *shm_name) {
...
template <typename T> class gaia_ptr {
...
public:
  ...
  static gaia_ptr<gaia_se_node> create(gaia_id_t id, gaia_type_t type,
...
struct gaia_se_edge {
  ...
  static gaia_ptr<gaia_se_edge> create(gaia_id_t id, gaia_type_t type,
