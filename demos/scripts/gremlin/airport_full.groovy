...
// By incoming flights.
g.V().hasLabel('airport').out().out('arrives_at').groupCount().by('name').order(local).by(values).unfold()
...
g.V().has('iata', 'SEA').repeat(out().out('arrives_at').as('hop')).times(2).has('iata', 'OTP').select(first, 'hop').dedup().values('name')

// Describe the 1 hop routes between SEA and OTP.
g.V().has('iata', 'SEA').repeat(out().out('arrives_at')).times(2).has('iata', 'OTP').path().by('iata').by(out('operated_by').values('icao'))

// Describe 1 stop routes between SEA and OTP that do not stop in UK.
g.V().has('iata', 'SEA').out().out('arrives_at').has('country', neq('United Kingdom')).out().out('arrives_at').has('iata', 'OTP').path().by('iata').by(out('operated_by').values('icao'))
