...
The relationship metadata needed for referential integrity check is stored in
`gaia_relationship` catalog table. Catalog records are also subject to the
referential integrity check which needs the information in `gaia_relationship`.
The current solution is to hard code the catalog's own relationship information
in `type_registry_t::init()` method. The method will populate the cache for
referential integrity check before the database records (including catalog
records) are available. After the catalog records are available, the cache will
be populated lazily from catalog records.
