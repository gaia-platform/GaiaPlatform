...
# see also human.py
def human(num, power="Ki", units=None, disable_scaling=False):
    if units is None:
        powers = ["Ki", "Mi", "Gi", "Ti"]
        while num >= 1000 and not disable_scaling:  # 4 digits
            num /= 1024.0
...
def print_memory_usage(
    ...
    for cmd in sorted_cmds:
        ...
        output_data = (
            human(cmd[1] - shareds[cmd[0]], disable_scaling=keep_original_units),
            human(shareds[cmd[0]], disable_scaling=keep_original_units),
            human(cmd[1], disable_scaling=keep_original_units),
        )
        if show_swap:
            output_string += "   %9s"
            output_data += (human(swaps[cmd[0]], disable_scaling=keep_original_units),)
        output_string += "\t%s\n"
