...
CREATE SERVER gaia
    FOREIGN DATA WRAPPER gaia_fdw
    OPTIONS (reset 'true');

CREATE SCHEMA airport_demo;

IMPORT FOREIGN SCHEMA airport_demo
    FROM SERVER gaia INTO airport_demo;
...
CREATE TABLE rawdata_airlines(
    al_id int primary key, name text, alias text, iata char(3), icao char(5),
...
CREATE TABLE airports_copy(
    gaia_id bigint,
    -- ap_id int primary key, name text, city text, country text, iata char(3) unique, icao char(4) unique,
    ap_id int primary key, name text, city text, country text, iata char(3), icao char(4) unique,
    latitude double precision, longitude double precision, altitude int, timezone float,
    dst char(1), tztext text, type text, source text);
