    ...
    int* fds, size_t fd_count, session_event_t event, const void*, session_state_t old_state, session_state_t new_state)
{
    ...
    ASSERT_PRECONDITION(seals == (F_SEAL_SHRINK | F_SEAL_GROW), "Unexpected seals on log fd!");
...
void server_t::recover_persistent_log()
{
    ...
    if (!(s_server_conf.persistence_mode() == persistence_mode_t::e_disabled))
    {
        if (!persistent_log_handler)
        {
            ...
            persistent_log_handler = make_unique<gaia::db::persistent_log_handler_t>(s_server_conf.data_dir());
            ...
            persistent_log_handler->register_write_to_persistent_store_fn(put_obj);
            persistent_log_handler->register_remove_from_persistent_store_fn(remove_obj);
...
void server_t::persistent_log_writer_handler()
{
    ...
    while (!shutdown)
    {
        ...
        for (int i = 0; i < ready_fd_count; ++i)
        {
            ...
            {
                if (ev.data.fd == s_signal_log_write_eventfd)
                {
                    throw_system_error("Txn write eventfd error!");
                }
            ...
            if (ev.data.fd == s_validate_persistence_batch_eventfd)
            {
                ...
                persistent_log_handler->validate_flushed_batch();
                write_to_persistent_log();
            }
...
void server_t::session_handler(int session_socket)
{
    ...
    s_session_unblock_eventfd = make_eventfd(0);

    auto cleanup_session_unblock_efd = make_scope_guard([]() {
        close_fd(s_session_unblock_eventfd);
    });
...
bool server_t::txn_commit()
{
    ...
    // Add eventfd in the log which the persistence thread will write to.
    s_log->session_unblock_fd = s_session_unblock_eventfd;
    ...
    if (persistent_log_handler)
    {
        ...
        {
            int err = errno;
            const char* reason = ::explain_read(s_session_unblock_eventfd, &val, sizeof(val));
...
void server_t::run(server_config_t server_conf)
{
    ...
    while (true)
    {
        ...
        s_validate_persistence_batch_eventfd = make_eventfd(0);
