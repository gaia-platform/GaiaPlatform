...
server_t::gc_guard_t::gc_guard_t(bool allow_recursion)
{
    ...
    bool server_t::reserve_safe_ts_entry()
    {
        ASSERT_PRECONDITION(
            s_safe_ts_entry_index == c_invalid_safe_ts_entry_index,
        ...
        size_t reserved_entry_index = c_invalid_safe_ts_entry_index;
        while (true)
        {
            reserved_entry_index = find_first_unset_bit(
                s_safe_ts_reserved_entries_bitmap, c_safe_ts_reserved_entries_bitmap_size_in_words);
            ...
            // infinite loop).
            if (reserved_entry_index == c_max_bit_index)
            {
                return false;
            }
            ...
            if (try_set_bit_value(
                    ...
                    c_safe_ts_reserved_entries_bitmap_size_in_words,
                    reserved_entry_index, true))
            {
                break;
            }
        }
        ...
        s_safe_ts_entry_index = reserved_entry_index;

        return true;
    }

    void server_t::release_safe_ts_entry()
    {
        ASSERT_PRECONDITION(
            s_safe_ts_entry_index != c_invalid_safe_ts_entry_index,
        ...
        ASSERT_PRECONDITION(
            is_bit_set(
                s_safe_ts_reserved_entries_bitmap,
                c_safe_ts_reserved_entries_bitmap_size_in_words,
                s_safe_ts_entry_index),
            "Expected the bit for this thread's safe_ts entry index to be set!");

        // Invalidate both of this thread's safe_ts entries.
        s_safe_ts_per_thread_entries[s_safe_ts_entry_index].first = c_invalid_gaia_txn_id;
        s_safe_ts_per_thread_entries[s_safe_ts_entry_index].second = c_invalid_gaia_txn_id;

        // Invalidate the thread-local copy of this entry's index before marking its
        // index "free".
        size_t safe_ts_entry_index = s_safe_ts_entry_index;
        s_safe_ts_entry_index = c_invalid_safe_ts_entry_index;
    ...
    bool server_t::reserve_safe_ts(gaia_txn_id_t safe_ts)
    {
        ...
        // should have been invalidated by a previous call to this method). Also,
        // any valid entry should still be ahead of the pre-truncate watermark
        // (otherwise we either failed to invalidate the entry of a safe_ts that
        // failed validation, or the safe_ts check in the truncation algorithm has a
        // bug).
        auto& entries = s_safe_ts_per_thread_entries[s_safe_ts_entry_index];
        ...
        for (gaia_txn_id_t entry : entries)
        {
            ASSERT_INVARIANT(
                ...
                "A published safe_ts entry cannot lag the pre-truncate watermark!");
        ...
        // (equality is acceptable because the post-GC watermark is an exclusive
        // upper bound for advancing the pre-truncate watermark).
    ...
    gaia_txn_id_t server_t::get_safe_truncation_ts()
    {
        ...
        //    watermark will later fail validation.)
        //
        ...
        //
        // If get_safe_truncation_ts() is called after reserve_safe_ts(safe_ts)
        // returns success, then we know that the scan of published safe_ts entries
        // will observe safe_ts, so safe_ts is an upper bound on the value returned
        // by get_safe_truncation_ts(). Furthermore, if validation succeeded, then
        // we know that safe_ts is at least as large as the validation-time value of
        // the post-GC watermark, so safe_ts is also at least as large as the
        // validation-time value of the pre-truncate watermark (if validation
        // failed, the pre-truncate watermark may already have advanced past
        // safe_ts). Once safe_ts has been published, it is an upper bound on any
        // future value returned by get_safe_truncation_ts(), so if safe_ts was at
        // least as large as the pre-truncate watermark when it was published
        // (ensured by validation), safe_ts is an upper bound on the pre-truncate
        // watermark at all times in the future.
        //
        // If a call to get_safe_truncation_ts() was in progress when
        // reserve_safe_ts(safe_ts) returned, then there are two possibilities:
        // either the published value of safe_ts was observed during the scan, or it
        // was not.
        //
        // If safe_ts was observed by the scan, then we know that it is an upper
        // bound on the return value of get_safe_truncation_ts(), so if validation
        // succeeded, then by the above argument, safe_ts is an upper bound on the
        // pre-truncate watermark at all times in the future.
        //
        // If safe_ts was not observed by the scan, we know that the pre-scan value
        // of the post-GC watermark must have been read before the validation-time
        // value of the post-GC watermark. Because watermarks are monotonically
        // nondecreasing, the validation-time value of the post-GC watermark is at
        // least as large as the pre-scan value of the post-GC watermark. Therefore,
        // since validation ensures that safe_ts is at least as large as the
        // validation-time value of the post-GC watermark, and the pre-scan value of
        // the post-GC watermark is an upper bound on the return value of
        // get_safe_truncation_ts(), safe_ts is an upper bound on the return value
        // of get_safe_truncation_ts(). If validation succeeded, we also know that
        // safe_ts is an upper bound on the validation-time value of the
        // pre-truncate watermark (because the post-GC watermark is an upper bound
        // on the pre-truncate watermark). Therefore, no other thread could have
        // advanced the pre-truncate watermark past safe_ts before validation
        // succeeded. So by induction, given that safe_ts is an upper bound on the
        // pre-truncate watermark at validation time and also on all future values
        // returned by get_safe_truncation_ts(), we know that safe_ts is an upper
        // bound on the pre-truncate watermark at all times in the future.
