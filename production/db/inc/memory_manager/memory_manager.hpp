...
class memory_manager : public base_memory_manager
{
    public:
    ...
    error_code allocate(size_t memorySize, ADDRESS_OFFSET& allocatedMemoryOffset) const;
    ...
    error_code create_stack_allocator(size_t memorySize, stack_allocator*& pStackAllocator) const;
    ...
    error_code commit_stack_allocator(
    ...
    error_code get_unserialized_allocations_list_head(memory_list_node*& pListHead) const;
    ...
    error_code update_unserialized_allocations_list_head(
    ...
    struct metadata
    {
        ...
        memory_record free_memory_list_head;
        memory_record reclaimed_records_list_head;
        memory_record unserialized_allocations_list_head;
    ...
    // Get a memory record.
    memory_record* get_memory_record() const;

    // Attempt to allocate a new record from our main memory block.
    memory_record* get_new_memory_record() const;

    // Attempt to allocate from one of the previously allocated memory records.
    memory_record* get_reclaimed_memory_record() const;

    // Insert free memory record in its proper place in the list of free memory records.
    void insert_free_memory_record(memory_record* pFreeMemoryRecord) const;

    // Insert memory record in the list of reclaimed memory records.
    void insert_reclaimed_memory_record(memory_record* pReclaimedMemoryRecord) const;

    // Insert unserialized allocations record in the list of unserialized allocations.
    void insert_unserialized_allocations_record(memory_record* pUnserializedAllocationsRecord) const;

    // Mark memory as free by setting up a free memory record to track it.
    memory_record* get_free_memory_record(ADDRESS_OFFSET memoryOffset, size_t memorySize) const;
    ...
    void process_free_memory_records(memory_record** freeMemoryRecords, size_t size, bool markAsFree) const;

    // Insert records in the list of free memory records.
    void insert_free_memory_records(memory_record** freeMemoryRecords, size_t size) const;

    // Insert records in the list of reclaimed memory records.
    void reclaim_records(memory_record** freeMemoryRecords, size_t size) const;

    // Track the StackAllocator metadata for future serialization.
    error_code track_stack_allocator_metadata_for_serialization(
    ...
    void output_list_content(memory_record listHead) const;
};
