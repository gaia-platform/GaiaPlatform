...
// The total number of 64B slots in a 4MB chunk can be represented using a 16-bit integer.
// A slot_offset_t value is just the low 16 bits of a 32-bit gaia_offset_t; it
// is the offset of an object in 64B units, relative to the beginning of its 4MB
// chunk, while a gaia_offset_t value is an object's offset in 64B units,
// relative to the base address of the data segment.
...
// The total number of 4MB chunks in 256GB memory can be represented using a 16-bit integer.
// A chunk_offset_t value is just the high 16 bits of a 32-bit gaia_offset_t; it is the offset
// of an object in 4MB units, relative to the base address of the data segment.
