...
gaia_ptr_server::gaia_ptr_server(const gaia_id_t id, const uint64_t size)
    ...
    hash_node->row_id = row_id = server::allocate_row_id(server::s_shared_offsets, server::s_data);
    server::allocate_object(row_id, size, server::s_shared_offsets, server::s_data);
}

object* gaia_ptr_server::to_ptr() const {
    ...
    return row_id && (*server::s_shared_offsets)[row_id]
        ? reinterpret_cast<object*>(server::s_data->objects + (*server::s_shared_offsets)[row_id])
        : nullptr;
}
