...
gaia_ptr& gaia_ptr::clone() {
    ...
    auto new_this = to_ptr();
    if (client::is_valid_event(new_this->container_id)) {
        client::s_events.emplace_back(event_type_t::row_insert, new_this->container_id, new_this->id, empty_position_list);
    }
...
gaia_ptr& gaia_ptr::update_payload(size_t data_size, const void* data) {
    ...
    if (client::is_valid_event(new_this->container_id)) {
        ...
        gaia::db::payload_types::compute_payload_diff(new_this->container_id, old_data_payload, (const uint8_t*)data, &position_list);
        client::s_events.emplace_back(event_type_t::row_update, new_this->container_id, new_this->id, position_list);
    }
...
void gaia_ptr::create_insert_trigger(gaia_container_id_t container_id, gaia_id_t id) {
    if (client::is_valid_event(container_id)) {
        client::s_events.emplace_back(event_type_t::row_insert, container_id, id, empty_position_list);
    }
}
...
void gaia_ptr::find_next(gaia_container_id_t container_id) {
    // search for rows of this container id within the range of used slots
...
void gaia_ptr::reset() {
    ...
    if (client::is_valid_event(to_ptr()->container_id)) {
        client::s_events.emplace_back(event_type_t::row_delete, to_ptr()->container_id, to_ptr()->id, empty_position_list);
    }
...
void gaia_ptr::add_child_reference(gaia_id_t child_id, reference_offset_t first_child_offset) {
    auto parent_container = container_id();
    auto parent_metadata = container_registry_t::instance().get_or_create(parent_container);
    auto relationship = parent_metadata.find_parent_relationship(first_child_offset);

    if (!relationship) {
        throw invalid_reference_offset(parent_container, first_child_offset);
    }
    ...
    if (relationship->parent_container != parent_container) {
        throw invalid_relationship_container(first_child_offset, parent_container, relationship->parent_container);
    }

    if (relationship->child_container != child_ptr.container_id()) {
        throw invalid_relationship_container(first_child_offset, child_ptr.container_id(), relationship->child_container);
    }
    ...
    if (references()[first_child_offset] != INVALID_GAIA_ID) {
        ...
        if (relationship->cardinality == cardinality_t::one) {
            throw single_cardinality_violation(parent_container, first_child_offset);
        }
    }
    ...
    if (child_ptr.references()[relationship->parent_offset] != INVALID_GAIA_ID) {
        ...
        throw child_already_referenced(child_ptr.container_id(), relationship->parent_offset);
    }
...
void gaia_ptr::add_parent_reference(gaia_id_t parent_id, reference_offset_t parent_offset) {
    auto child_container = container_id();

    auto child_metadata = container_registry_t::instance().get_or_create(child_container);
    auto child_relationship = child_metadata.find_child_relationship(parent_offset);

    if (!child_relationship) {
        throw invalid_reference_offset(child_container, parent_offset);
    }
