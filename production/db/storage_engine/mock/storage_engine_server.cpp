...
const uint64_t gaia_server::MAX_SEMAPHORE_COUNT = 0xfffffffffffffffe;
int gaia_server::s_server_shutdown_event_fd = -1;
int gaia_server::s_connect_socket = -1;
std::mutex gaia_server::s_commit_lock;
int gaia_server::s_fd_data = -1;
gaia_mem_base::offsets* gaia_server::s_shared_offsets = nullptr;
thread_local SessionState gaia_server::s_session_state = SessionState::DISCONNECTED;
thread_local bool gaia_server::s_session_shutdown = false;
constexpr gaia_server::ValidTransition gaia_server::s_valid_transitions[];

void gaia_server::handle_connect(int*, size_t, SessionEvent event, SessionState old_state, SessionState new_state) {
    assert(event == SessionEvent::CONNECT);
    // This message should only be received after the client thread was first initialized.
    assert(old_state == SessionState::DISCONNECTED && new_state == SessionState::CONNECTED);
    // We need to reply to the client with the fds for the data/locator segments.
    FlatBufferBuilder builder;
    build_server_reply(builder, SessionEvent::CONNECT, old_state, new_state);
...
void gaia_server::handle_begin_txn(int*, size_t, SessionEvent event, SessionState old_state, SessionState new_state) {
    assert(event == SessionEvent::BEGIN_TXN);
    // This message should only be received while a transaction is in progress.
    assert(old_state == SessionState::CONNECTED && new_state == SessionState::TXN_IN_PROGRESS);
    // Currently we don't need to alter any server-side state for opening a transaction.
}

void gaia_server::handle_abort_txn(int*, size_t, SessionEvent event, SessionState old_state, SessionState new_state) {
    assert(event == SessionEvent::ABORT_TXN);
    // This message should only be received while a transaction is in progress.
    assert(old_state == SessionState::TXN_IN_PROGRESS && new_state == SessionState::CONNECTED);
    // Currently we don't need to alter any server-side state for rolling back a transaction.
}

void gaia_server::handle_commit_txn(int* fds, size_t fd_count, SessionEvent event, SessionState old_state, SessionState new_state) {
    assert(event == SessionEvent::COMMIT_TXN);
    // This message should only be received while a transaction is in progress.
    assert(old_state == SessionState::TXN_IN_PROGRESS && new_state == SessionState::TXN_COMMITTING);
    // Get the log fd and mmap it.
    assert(fds && fd_count == 1);
    int fd_log = *fds;
    ...
    SessionEvent decision = success ? SessionEvent::DECIDE_TXN_COMMIT : SessionEvent::DECIDE_TXN_ABORT;
    // Server-initiated state transition! (Any issues with reentrant handlers?)
    apply_transition(decision, nullptr, 0);
}

void gaia_server::handle_decide_txn(int*, size_t, SessionEvent event, SessionState old_state, SessionState new_state) {
    assert(event == SessionEvent::DECIDE_TXN_COMMIT || event == SessionEvent::DECIDE_TXN_ABORT);
    assert(old_state == SessionState::TXN_COMMITTING && new_state == SessionState::CONNECTED);
    FlatBufferBuilder builder;
    build_server_reply(builder, event, old_state, new_state);
    send_msg_with_fds(s_session_socket, nullptr, 0, builder.GetBufferPointer(), builder.GetSize());
}

void gaia_server::handle_client_shutdown(int*, size_t, SessionEvent event, SessionState, SessionState new_state) {
    assert(event == SessionEvent::CLIENT_SHUTDOWN);
    assert(new_state == SessionState::DISCONNECTED);
...
void gaia_server::handle_server_shutdown(int*, size_t, SessionEvent event, SessionState, SessionState new_state) {
    assert(event == SessionEvent::SERVER_SHUTDOWN);
    assert(new_state == SessionState::DISCONNECTING);
