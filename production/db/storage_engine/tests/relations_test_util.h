...
constexpr gaia_container_id_t c_doctor_type = 1;
constexpr gaia_container_id_t c_patient_type = 2;
constexpr gaia_container_id_t c_non_existent_type = 1001;
...
public:
    ...
    void create_relationship() {
        if (m_parent_type == INVALID_GAIA_CONTAINER) {
            throw invalid_argument("parent_container must be set");
        }

        if (m_child_type == INVALID_GAIA_CONTAINER) {
        ...
        auto rel = make_shared<relationship_t>(relationship_t{
            .parent_container = this->m_parent_type,
            .child_container = this->m_child_type,
        ...
        auto& parent_meta = container_registry_t::instance().get_or_create(m_parent_type);
        parent_meta.add_parent_relationship(m_first_child_offset, rel);

        auto& child_meta = container_registry_t::instance().get_or_create(m_child_type);
...
private:
    ...
    gaia_container_id_t m_parent_type = INVALID_GAIA_CONTAINER;
    gaia_container_id_t m_child_type = INVALID_GAIA_CONTAINER;
 ...
 */
gaia_ptr create_object(gaia_container_id_t type, size_t data_size, const void* data) {
...
gaia_ptr create_object(gaia_container_id_t type, std::string payload) {
