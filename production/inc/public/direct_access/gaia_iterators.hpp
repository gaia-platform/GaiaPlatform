...
struct gaia_container_t {
    static gaia_iterator_t<shared_ptr<T_gaia>> begin() { return gaia_iterator_t<shared_ptr<T_gaia>>(T_gaia::get_first()); }
    static gaia_iterator_t<shared_ptr<T_gaia>> end() { return gaia_iterator_t<shared_ptr<T_gaia>>(nullptr); }
};
...
class gaia_set_iterator_t {
    shared_ptr<T_foreign> m_edc_ptr;
public:
    gaia_set_iterator_t() {}
    gaia_set_iterator_t(shared_ptr<T_foreign> edc_ptr) { m_edc_ptr = edc_ptr; }

    shared_ptr<T_foreign> operator*() { return m_edc_ptr; }

    gaia_set_iterator_t& operator++();

    bool operator!=(const gaia_set_iterator_t&) const { return m_edc_ptr.get() != nullptr; }
};
...
public:
    ...
    void insert(shared_ptr<T_foreign> foreign_ptr);

    void erase(shared_ptr<T_foreign> foreign_ptr);
};
