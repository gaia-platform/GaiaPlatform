...
struct server_instance_conf_t
{
    ...
    static server_instance_conf_t get_default()
    {
        return server_instance_conf_t{
            .server_exec_path = get_server_exec_path(),
...
public:
    ...
    // TODO deprecated
    bool is_server_process_alive()
    {
        for (int i = 0; i < 10; i++)
        {
            if (::kill(m_server_pid, SIGKILL) == EXIT_SUCCESS)
            {
                return true;
            }
            gaia_log::sys().debug("Checking if server instance {} with pid:{} is alive...", instance_name(), m_server_pid);
            ::usleep(100000);
        }
        return false;
    }
