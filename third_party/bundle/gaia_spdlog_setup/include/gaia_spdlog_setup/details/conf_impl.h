/**
 * Implementation of non-public facing functions in gaia_spdlog_setup.
...
#include "gaia_spdlog/sinks/daily_file_sink.h"
#include "gaia_spdlog/sinks/null_sink.h"
#include "gaia_spdlog/sinks/rotating_file_sink.h"
#include "gaia_spdlog/sinks/sink.h"
#include "gaia_spdlog/sinks/stdout_sinks.h"

#ifdef _WIN32
#include "gaia_spdlog/sinks/msvc_sink.h"
#include "gaia_spdlog/sinks/wincolor_sink.h"
#else
#include "gaia_spdlog/sinks/ansicolor_sink.h"
#include "gaia_spdlog/sinks/syslog_sink.h"
#endif
...
static constexpr auto ASYNC_OVERFLOW_POLICY =
    gaia_spdlog::async_overflow_policy::block;
...
const std::unordered_map<std::string, gaia_spdlog::async_overflow_policy>
    ASYNC_OVERFLOW_POLICY_MAP{{
        {names::BLOCK, gaia_spdlog::async_overflow_policy::block},
        {names::OVERRUN_OLDEST, gaia_spdlog::async_overflow_policy::overrun_oldest},
    }};
...
inline void set_sink_level_if_present(
    const std::shared_ptr<cpptoml::table> &sink_table,
    const std::shared_ptr<gaia_spdlog::sinks::sink> &sink) {
...
inline auto sink_from_sink_type(
    ...
    using gaia_spdlog::sinks::basic_file_sink_st;
    using gaia_spdlog::sinks::daily_file_sink_mt;
    using gaia_spdlog::sinks::daily_file_sink_st;
    using gaia_spdlog::sinks::null_sink_mt;
    using gaia_spdlog::sinks::null_sink_st;
    using gaia_spdlog::sinks::rotating_file_sink_mt;
    using gaia_spdlog::sinks::rotating_file_sink_st;
    using gaia_spdlog::sinks::sink;
    using gaia_spdlog::sinks::stderr_sink_mt;
    using gaia_spdlog::sinks::stderr_sink_st;
    using gaia_spdlog::sinks::stdout_sink_mt;
    using gaia_spdlog::sinks::stdout_sink_st;

    // std
    using std::make_shared;

#ifdef _WIN32
    using color_stdout_sink_st = gaia_spdlog::sinks::wincolor_stdout_sink_st;
    using color_stdout_sink_mt = gaia_spdlog::sinks::wincolor_stdout_sink_mt;
    using color_stderr_sink_st = gaia_spdlog::sinks::wincolor_stderr_sink_st;
    using color_stderr_sink_mt = gaia_spdlog::sinks::wincolor_stderr_sink_mt;
    using msvc_sink_st = gaia_spdlog::sinks::msvc_sink_st;
    using msvc_sink_mt = gaia_spdlog::sinks::msvc_sink_mt;
#else
    using color_stdout_sink_st = gaia_spdlog::sinks::ansicolor_stdout_sink_st;
    using color_stdout_sink_mt = gaia_spdlog::sinks::ansicolor_stdout_sink_mt;
    using color_stderr_sink_st = gaia_spdlog::sinks::ansicolor_stderr_sink_st;
    using color_stderr_sink_mt = gaia_spdlog::sinks::ansicolor_stderr_sink_mt;

    using gaia_spdlog::sinks::syslog_sink_mt;
    using gaia_spdlog::sinks::syslog_sink_st;
#endif
...
inline void set_logger_level_if_present(
    ...
    const std::shared_ptr<gaia_spdlog::logger> &logger) {
...
inline auto setup_sinks(const std::shared_ptr<cpptoml::table> &config)
    -> std::unordered_map<std::string, std::shared_ptr<gaia_spdlog::sinks::sink>> {
    ...
    unordered_map<string, shared_ptr<gaia_spdlog::sinks::sink>> sinks_map;
...
setup_thread_pools(const std::shared_ptr<cpptoml::table> &config) -> std::
    unordered_map<std::string, std::shared_ptr<gaia_spdlog::details::thread_pool>> {
    ...
    using gaia_spdlog::details::thread_pool;
...
inline auto setup_sync_logger(
    ...
    const std::vector<std::shared_ptr<gaia_spdlog::sinks::sink>> &logger_sinks)
    -> std::shared_ptr<gaia_spdlog::logger> {
    return std::make_shared<gaia_spdlog::logger>(
...
inline auto setup_async_logger(
    ...
    const std::unordered_map<
        std::string,
        std::shared_ptr<gaia_spdlog::details::thread_pool>> &thread_pools_map)
    -> std::shared_ptr<gaia_spdlog::logger> {
    ...
    return std::make_shared<gaia_spdlog::async_logger>(
        name,
        logger_sinks.cbegin(),
        logger_sinks.cend(),
...
inline auto setup_logger(
    const std::shared_ptr<cpptoml::table> &logger_table,
    const std::unordered_map<std::string, std::shared_ptr<gaia_spdlog::sinks::sink>>
        &sinks_map,
    ...
    std::vector<std::shared_ptr<gaia_spdlog::sinks::sink>> logger_sinks;
    ...
    std::shared_ptr<gaia_spdlog::logger> logger;
