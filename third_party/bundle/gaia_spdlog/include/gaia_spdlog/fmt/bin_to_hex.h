...
// logger->info("Some buffer {}", gaia_spdlog::to_hex(v));
// char buf[128];
// logger->info("Some buffer {:X}", gaia_spdlog::to_hex(std::begin(buf), std::end(buf)));
// logger->info("Some buffer {:X}", gaia_spdlog::to_hex(std::begin(buf), std::end(buf), 16));
...
struct formatter<gaia_spdlog::details::dump_info<T>>
{
    ...
    auto format(const gaia_spdlog::details::dump_info<Container> &the_range, FormatContext &ctx) -> decltype(ctx.out())
    {
        GAIA_SPDLOG_CONSTEXPR const char *hex_upper = "0123456789ABCDEF";
