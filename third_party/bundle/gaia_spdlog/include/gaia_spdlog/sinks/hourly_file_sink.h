...
struct hourly_filename_calculator
{
    ...
    static filename_t calc_filename(const filename_t &filename, const tm &now_tm)
    {
        ...
        return gaia_fmt::format(
            GAIA_SPDLOG_FILENAME_T("{}_{:04d}{:02d}{:02d}_{:02d}{}"), basename, now_tm.tm_year + 1900, now_tm.tm_mon + 1, now_tm.tm_mday, now_tm.tm_hour, ext);
    }
};
...
private:
    ...
    tm now_tm(log_clock::time_point tp)
    {
        ...
        return gaia_spdlog::details::os::localtime(tnow);
    }
    ...
    // Throw gaia_spdlog_ex on failure to delete the old file.
    void delete_old_()
    {
        ...
        if (filenames_q_.full())
        {
            ...
            if (!ok)
            {
                ...
                GAIA_SPDLOG_THROW(gaia_spdlog_ex("Failed removing hourly file " + filename_to_str(old_filename), errno));
            }
        }
        filenames_q_.push_back(std::move(current_file));
    }
