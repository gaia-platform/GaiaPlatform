...
static QualType mapFieldType(catalog::data_type_t dbType, ASTContext *context) {
  // Clang complains if we add a default clause to a switch that covers all
  // values of an enum, so this code is written to avoid that.
...
bool Sema::does_path_includes_tags(const std::vector<std::string> &path,
                                   SourceLocation loc) {
  std::unordered_map<std::string, std::string> tagMapping =
      getTagMapping(getCurFunctionDecl(), loc);
  if (tagMapping.empty()) {
    return false;
  }
  for (const auto &path_component : path) {
    if (tagMapping.find(get_table_from_expression(path_component)) !=
        tagMapping.end()) {
      return true;
    }
  }
...
std::string Sema::ParseExplicitPath(const std::string &pathString,
                                    SourceLocation loc) {
  ...
  while (tagPosition != string::npos || arrowPosition != string::npos) {
    if (tagPosition < arrowPosition) {
      ...
      tag = pathString.substr(searchStartPosition,
                              tagPosition - searchStartPosition);
      ...
      if (tagMap.find(tag) != tagMap.end() ||
          tagMapping.find(tag) != tagMapping.end()) {
      ...
      string table = pathString.substr(searchStartPosition,
                                       arrowPosition - searchStartPosition);
  ...
  // If explicit path has one component only, this component will be checked at
  // later stage Therefore there is no need to perform more checks here.
  if (path.size() > 1 || pathString.front() == '/' || !tagMap.empty() ||
      does_path_includes_tags(path, loc)) {
    unordered_multimap<string, TableLinkData_t> relationData =
        getCatalogTableRelations(loc);
    ...
    for (string pathComponent : path) {
      if (duplicate_component_check_set.find(pathComponent) !=
          duplicate_component_check_set.end()) {
        Diag(loc, diag::err_invalid_explicit_path);
        return "";
      } else {
      ...
      if (!previousTable.empty()) {
        ...
        if (relatedTablesIterator.first == relationData.end() &&
          ...
          Diag(loc, diag::err_no_relations_table_in_path)
              << previousTable << pathComponent;
          return "";
        }

        bool isMatchFound = false;
        for (auto tableIterator = relatedTablesIterator.first;
             tableIterator != relatedTablesIterator.second; ++tableIterator) {
...
unordered_map<string, unordered_map<string, QualType>>
Sema::getTableData(SourceLocation loc) {
  ...
  try {
    ...
    for (const catalog::gaia_field_t &field : catalog::gaia_field_t::list()) {
      ...
      fields[field.name()] = mapFieldType(
          static_cast<catalog::data_type_t>(field.type()), &Context);
...
unordered_multimap<string, Sema::TableLinkData_t>
Sema::getCatalogTableRelations(SourceLocation loc) {
...
void Sema::addField(IdentifierInfo *name, QualType type, RecordDecl *RD,
                    SourceLocation loc) const {
  FieldDecl *Field =
      FieldDecl::Create(Context, RD, loc, loc, name, type, /*TInfo=*/nullptr,
...
void Sema::addMethod(IdentifierInfo *name, DeclSpec::TST retValType,
                     DeclaratorChunk::ParamInfo *Params, unsigned NumParams,
                     AttributeFactory &attrFactory, ParsedAttributes &attrs,
                     Scope *S, RecordDecl *RD, SourceLocation loc) {
  ...
  D.getMutableDeclSpec().SetTypeSpecType(retValType, loc, dummy, diagId,
                                         getPrintingPolicy());
  ...
  D.AddTypeInfo(DeclaratorChunk::getFunction(
                    true, false, loc, Params, NumParams, loc, loc, true, loc,
                    /*MutableLoc=*/loc, EST_None, SourceRange(), nullptr,
                    nullptr, 0, nullptr, nullptr, None, loc, loc, D,
                    TypeResult(), &DS),
                std::move(attrs), loc);
  ...
  CXXMethodDecl *Ret = CXXMethodDecl::Create(
      Context, cast<CXXRecordDecl>(RD), SourceLocation(), NameInfo,
      tInfo->getType(), tInfo, SC_None, false, false, SourceLocation());
...
QualType Sema::getRuleContextType(SourceLocation loc) {
  ...
  // insert fields
  addField(&Context.Idents.get("ruleset_name"),
           Context.getPointerType((Context.CharTy.withConst()).withConst()), RD,
           loc);
  addField(&Context.Idents.get("rule_name"),
           Context.getPointerType((Context.CharTy.withConst()).withConst()), RD,
           loc);
  addField(&Context.Idents.get("event_type"), Context.UnsignedIntTy.withConst(),
           RD, loc);
  addField(&Context.Idents.get("gaia_type"), Context.UnsignedIntTy.withConst(),
           RD, loc);
...
QualType Sema::getTableType(const std::string &tableName, SourceLocation loc) {
  ...
  unordered_map<string, unordered_map<string, QualType>> tableData =
      getTableData(loc);
  if (tableData.empty()) {
    return Context.VoidTy;
  }
  ...
  if (realType != nullptr) {
    ...
    DeclarationName Name =
        Context.DeclarationNames.getCXXConversionFunctionName(ClassType);
    DeclarationNameInfo NameInfo(Name, loc);

    auto conversionFunctionDeclaration = CXXConversionDecl::Create(
        Context, cast<CXXRecordDecl>(RD), loc, NameInfo, R, nullptr, false,
  ...
  addMethod(&Context.Idents.get("delete_row"), DeclSpec::TST_void, nullptr, 0,
            attrFactory, attrs, &S, RD, loc);
  addMethod(&Context.Idents.get("gaia_id"), DeclSpec::TST_int, nullptr, 0,
            attrFactory, attrs, &S, RD, loc);
...
QualType Sema::getFieldType(const std::string &fieldName, SourceLocation loc) {
  ...
  if (tableData.find(fieldName) != tableData.end() ||
      tagMapping.find(fieldName) != tagMapping.end()) {
...
static bool parse_tagged_attribute(const string &attribute, string &table,
                                   string &tag) {
...
Sema::getTagMapping(const DeclContext *context, SourceLocation loc) {
  ...
  const GaiaOnUpdateAttr *update_attribute =
      ruleContext->getAttr<GaiaOnUpdateAttr>();
  const GaiaOnInsertAttr *insert_attribute =
      ruleContext->getAttr<GaiaOnInsertAttr>();
  const GaiaOnChangeAttr *change_attribute =
      ruleContext->getAttr<GaiaOnChangeAttr>();
  ...
  for (const auto &explicitPathTagMapIterator :
       extendedExplicitPathTagMapping) {
    const auto &tagMap = explicitPathTagMapIterator.second;
    for (const auto &tagMapIterator : tagMap) {
      if (explicitPathTagMapping.find(explicitPathTagMapIterator.first) ==
          explicitPathTagMapping.end()) {
        if (retVal.find(tagMapIterator.first) != retVal.end()) {
          Diag(loc, diag::err_tag_redefined) << tagMapIterator.first;
          return std::unordered_map<std::string, std::string>();
        } else {
...
NamedDecl *Sema::injectVariableDefinition(IdentifierInfo *II,
  ...
  DeclContext *context = getCurFunctionDecl();
  VarDecl *varDecl =
      VarDecl::Create(Context, context, loc, loc, II, qualType,
                      Context.getTrivialTypeSourceInfo(qualType, loc), SC_None);
  varDecl->setLexicalDeclContext(context);
  varDecl->setImplicit();

  varDecl->addAttr(GaiaFieldAttr::CreateImplicit(Context));
  varDecl->addAttr(FieldTableAttr::CreateImplicit(
      Context, &Context.Idents.get(fieldTableName)));
  ...
  if (GetExplicitPathData(loc, startLocation, endLocation, path)) {
    ...
    varDecl->addAttr(GaiaExplicitPathAttr::CreateImplicit(
        Context, path, startLocation.getRawEncoding(),
        endLocation.getRawEncoding(), argPathComponents.data(),
        argPathComponents.size()));
    varDecl->addAttr(GaiaExplicitPathTagKeysAttr::CreateImplicit(
        Context, argTagKeys.data(), argTagKeys.size()));
    varDecl->addAttr(GaiaExplicitPathTagValuesAttr::CreateImplicit(
        Context, argTagTables.data(), argTagTables.size()));
    varDecl->addAttr(GaiaExplicitPathDefinedTagKeysAttr::CreateImplicit(
        Context, argDefinedTagKeys.data(), argDefinedTagKeys.size()));
    varDecl->addAttr(GaiaExplicitPathDefinedTagValuesAttr::CreateImplicit(
        Context, argDefinedTagTables.data(), argDefinedTagTables.size()));
  }
...
Decl *Sema::ActOnRulesetDefStart(Scope *S, SourceLocation RulesetLoc,
  ...
  RulesetDecl *ruleset =
      RulesetDecl::Create(Context, CurContext, RulesetLoc, IdentLoc, Ident);
...
void Sema::AddExplicitPathData(SourceLocation location,
...
bool Sema::GetExplicitPathData(SourceLocation location,
                               SourceLocation &startLocation,
                               SourceLocation &endLocation,
...
bool Sema::RemoveTagData(SourceRange range) {
  if (range.isValid()) {
    auto startLocationIterator =
        extendedExplicitPathTagMapping.lower_bound(range.getBegin());
    auto endLocationIterator =
        extendedExplicitPathTagMapping.upper_bound(range.getEnd());
    if (startLocationIterator == extendedExplicitPathTagMapping.end() &&
        endLocationIterator == extendedExplicitPathTagMapping.end()) {
      return false;
    }
    extendedExplicitPathTagMapping.erase(startLocationIterator,
                                         endLocationIterator);
  }
  return true;
}
